...
class ArknightsPlugin(Star):
    ...
    async def on_message(self, event: AstrMessageEvent) -> MessageEventResult:
        ...
        try:
            ...
            if any(keyword in text for keyword in ["色色", "色图", "涩涩"]):
    ...
    async def handle_image_request(self, event: AstrMessageEvent) -> MessageEventResult:
        ...
        try:
            ...
            if not results:
                return event.plain_result("皇上不行了 喵")

            item = results[0]
            original_url = item['urls'].get("original")
            if not original_url:
                return event.plain_result("皇上没戴那个 喵")
            ...
            if not save_success:
                return event.plain_result("皇上不能内射 喵")
            ...
            try:
                ...
                return event.plain_result("啊 出来了 喵") if delete_success \
                ...
                return event.plain_result("皇上我处理不好 喵")
